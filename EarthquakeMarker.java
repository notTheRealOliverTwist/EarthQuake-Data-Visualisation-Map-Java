...
public abstract class EarthquakeMarker extends CommonMarker implements Comparable<EarthquakeMarker> {
	...
	public void showTitle(PGraphics pg, float x, float y) {
		...
		pg.fill(255);
		pg.rect(x, y - 25, widthOfTitle+20, 40);
		pg.fill(0);
		pg.text(title, x, y);
	}
	...
	public int compareTo(EarthquakeMarker marker) {
		// Might have to swap the equality operators around. Will see. Not sure
		// which object gets compared first.
		// Reverse order will also alter the operator order.
		// Will have to see which object is being compared first.
		// Additionally the values may be reversed, i.e -1 and positive one
		
		if(marker.getMagnitude() > this.getMagnitude()) {
			return 1;
		}
		return marker.getMagnitude() < this.getMagnitude()? -1 : 0;
	}
